...
pub fn crc32c_slice8(data: &[u8]) -> u32 {
    ...
    for chunk in chunks {
        ...
        crc = CRC32C_TABLES_8[7][(b0 ^ (crc & 0xff) as u8) as usize]
            ^ CRC32C_TABLES_8[6][(b1 ^ ((crc >> 8) & 0xff) as u8) as usize]
            ^ CRC32C_TABLES_8[5][(b2 ^ ((crc >> 16) & 0xff) as u8) as usize]
            ^ CRC32C_TABLES_8[4][(b3 ^ ((crc >> 24) & 0xff) as u8) as usize]
            ^ CRC32C_TABLES_8[3][b4 as usize]
            ^ CRC32C_TABLES_8[2][b5 as usize]
            ^ CRC32C_TABLES_8[1][b6 as usize]
            ^ CRC32C_TABLES_8[0][b7 as usize];
    }
